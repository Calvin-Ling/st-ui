...
- Fixed a bug that caused empty `<st-radio-group>` elements to log an error in the console [#1795]
...
- Improved the performance of `<st-include>` to prevent an apparent memory leak in some browsers [#1284]
...
- Added the `st-focus` and `st-blur` events to `<st-color-picker>`
...
- Fixed a bug in `<st-checkbox>` and `<st-radio>` that caused the checked icons to not scale property when resized
...
- Added `tag-base`, `tag-content`, and `tag-remove-button` parts to `<st-select>` [#682]
...
- Fixed a bug where pressing up/down in `<st-tab-group>` didn't select the next/previous tab in vertical placements
...
- Fixed a bug in `<st-button>` and `<st-icon-button>` where an unintended `ref` attribute was present
...
- 🚨 BREAKING: `<st-radio>` components must be located inside an `<st-radio-group>` for proper accessibility [#218]
...
- 🚨 BREAKING: removed `getAnimationNames()` and `getEasingNames()` methods from `<st-animation>` (you can import them from `utilities/animation.js` instead)
- 🚨 BREAKING: removed the `<st-icon-library>` component since it required imperative initialization (you can import the `registerIconLibrary()` function from `utilities/icon-library.js` instead)
- 🚨 BREAKING: removed the experimental `<st-theme>` component due to technical limitations (you should set the `st-theme-{name}` class on the `<body>` instead)
...
- 🚨 BREAKING: changed `getSymbol` property in `<st-rating>` to `symbol` (it now accepts a string or a function that returns an icon name)
...
- Fixed `min` and `max` types in `<st-input>` to allow numbers and strings [#330]
- Fixed a bug where `<st-checkbox>`, `<st-radio>`, and `<st-switch>` controls would shrink with long labels [#325]
- Fixed a bug in `<st-select>` where the dropdown menu wouldn't reposition when the box resized [#340]
...
- Added `handle-icon` slot to `<st-image-comparer>` [#311]
- Added `label` and `helpText` props and slots to `<st-range>` [#318]
- Added "Integrating with NextJS" tutorial to the docs, courtesy of [crutchcorn](https://github.com/crutchcorn)
- Added `content` slot to `<st-tooltip>` [#322]
- Fixed a bug in `<st-select>` where removing a tag would toggle the dropdown
- Fixed a bug in `<st-input>` and `<st-textarea>` where the input might not exist when the value watcher is called [#313]
- Fixed a bug in `<st-details>` where hidden elements would receive focus when tabbing [#323]
- Fixed a bug in `<st-icon>` where `st-error` would only be emitted for network failures [#326]
- Reduced the default line-height for `<st-tooltip>`
- Updated `<st-menu-item>` focus styles
- Updated `<st-select>` so tags will wrap when `multiple` is true
...
- Added `st-initial-focus` event to `<st-dialog>` and `<st-drawer>` so focus can be customized to a specific element
- Added `close-button` part to `<st-tab>` so the close button can be customized
- Added `scroll-button` part to `<st-tab-group>` so the scroll buttons can be customized
- Fixed a bug where `st-hide` would be emitted twice when closing an alert with `hide()`
- Fixed a bug in `<st-color-picker>` where the toggle button was smaller than the preview button in Safari
- Fixed a bug in `<st-tab-group>` where activating a nested tab group didn't work properly [#299]
- Fixed a bug in `<st-tab-group>` where removing tabs would throw an error
- Fixed a bug in `<st-alert>`, `<st-dialog>`, `<st-drawer>`, `<st-select>`, and `<st-tag>` where the close button's base wasn't exported so it couldn't be styled
- Removed `text` type from `<st-badge>` as it was erroneously copied and never had styles
- Updated `<st-tab-group>` so the `active` property is reflected to its attribute
...
- 🚨 BREAKING: Refactored `<st-select>` to improve accessibility [#216]
  - Removed the internal `<st-input>` because it was causing problems with a11y and virtual keyboards
  - Removed `input`, `prefix` and `suffix` parts
- 🚨 BREAKING: Removed `copy-button` part from `<st-color-picker>` since copying is now done by clicking the preview
- Added `getFormattedValue()` method to `<st-color-picker>` so you can retrieve the current value in any format
- Added visual separators between solid buttons in `<st-button-group>`
- Added `help-text` attribute to `<st-input>`, `<st-textarea>`, and `<st-select>`
- Fixed a bug where moving the mouse while `<st-dropdown>` is closing would remove focus from the trigger
- Fixed a bug where `<st-menu-item>` didn't set a default color in the dark theme
- Fixed a bug where `<st-color-picker>` preview wouldn't update in Safari
...
- Improved accessibility in `<st-tooltip>` by allowing escape to dismiss it [#219]
- Improved slot detection in `<st-card>`, `<st-dialog>`, and `<st-drawer>`
- Made `@types/resize-observer-browser` a dependency so users don't have to install it manually
- Refactored internal label + help text logic into a functional component used by `<st-input>`, `<st-textarea>`, and `<st-select>`
- Removed `st-blur` and `st-focus` events from `<st-menu>` since menus can't have focus as of 2.0.0-beta.22
- Updated `<st-spinner>` so the indicator is more obvious
- Updated to Bootstrap Icons 1.2.2

## 2.0.0-beta.24

- Added `<st-format-date>` component
- Added `indeterminate` state to `<st-progress-bar>` [#274]
- Added `--track-color`, `--indicator-color`, and `--label-color` to `<st-progress-bar>` [#276]
- Added `allow-scripts` attribute to `<st-include>` [#280]
- Fixed a bug where `<st-menu-item>` color variable was incorrect [#272]
- Fixed a bug where `<st-dialog>` and `<st-drawer>` would emit the `st-hide` event twice [#275]
- Fixed a bug where calling `event.preventDefault()` on certain form elements wouldn't prevent `<st-form>` from submitting [#277]
- Fixed drag handle orientation in `<st-image-comparer>`
- Restyled `<st-spinner>` so the track is visible and the indicator is smaller.
...
- Upgraded the status of `<st-form>`, `<st-image-comparer>`, and `<st-include>` from experimental to stable

## 2.0.0-beta.23

- Added `<st-format-number>` component
- Added `<st-relative-time>` component
- Added `closable` attribute to `<st-tab>`
- Added experimental `<st-resize-observer>` utility
- Added experimental `<st-theme>` utility and updated theming documentation
- Fixed a bug where `<st-menu-item>` wouldn't render properly in the dark theme
- Fixed a bug where `<st-select>` would show an autocomplete menu
...
- 🚨 BREAKING: Refactored `<st-menu>` and `<st-menu-item>` to improve accessibility by using proper focus states [#217]
  - Moved `tabindex` from `<st-menu>` to `<st-menu-item>`
  - Removed the `active` attribute from `<st-menu-item>` because synthetic focus states are bad for accessibility
  - Removed the `st-activate` and `st-deactivate` events from `<st-menu-item>` (listen for `focus` and `blur` instead)
  - Updated `<st-select>` so keyboard navigation still works
- Added `no-scroll-controls` attribute to `<st-tab-group>` [#253]
- Fixed a bug where setting `open` initially wouldn't show `<st-dialog>` or `<st-drawer>` [#255]
- Fixed a bug where `disabled` could be set when buttons are rendered as links
- Fixed a bug where hoisted dropdowns would render in the wrong position when placed inside `<st-dialog>` [#252]
- Fixed a bug where boolean aria attributes didn't explicitly set `true|false` string values in the DOM
- Fixed a bug where `aria-describedby` was never set on tooltip targets in `<st-tooltip>`
- Fixed a bug where setting `position` on `<st-image-comparer>` wouldn't update the divider's position
- Fixed a bug where the check icon was announced to screen readers in `<st-menu-item>`
- Improved `<st-icon-button>` accessibility by encouraging proper use of `label` and hiding the internal icon from screen readers [#220]
- Improved `<st-dropdown>` accessibility by attaching `aria-haspopup` and `aria-expanded` to the slotted trigger
- Refactored position logic to remove an unnecessary state variable in `<st-image-comparer>`
...
- Added `label` slot to `<st-input>`, `<st-select>`, and `<st-textarea>` [#248]
- Added `label` slot to `<st-dialog>` and `<st-drawer>`
- Added experimental `<st-include>` component
- Added status code to the `st-error` event in `<st-icon>`
- Fixed a bug where initial transitions didn't show in `<st-dialog>` and `<st-drawer>` [#247]
- Fixed a bug where indeterminate checkboxes would maintain the indeterminate state when toggled
- Fixed a bug where concurrent active modals (i.e. dialog, drawer) would try to steal focus from each other
- Improved `<st-color-picker>` grid and slider handles [#246]
- Refactored `<st-icon>` request logic and removed unused cache map
- Reworked show/hide logic in `<st-alert>`, `<st-dialog>`, and `<st-drawer>` to not use reflow hacks and the `hidden` attribute
- Reworked slot logic in `<st-card>`, `<st-dialog>`, and `<st-drawer>`
- Updated to Popper 2.5.3 to address a fixed position bug in Firefox

## 2.0.0-beta.20

- 🚨 BREAKING: Transformed all Shoelace events to lowercase ([details](#why-did-event-names-change))
- Added support for dropdowns and non-icon elements to `<st-input>`
- Added `spellcheck` attribute to `<st-input>`
- Added `<st-icon-library>` to allow custom icon library registration
- Added `library` attribute to `<st-icon>` and `<st-icon-button>`
- Added "Integrating with Rails" tutorial to the docs, courtesy of [ParamagicDev](https://github.com/ParamagicDev)
- Fixed a bug where `<st-progress-ring>` rendered incorrectly when zoomed in Safari [#227]
- Fixed a bug where tabbing into slotted elements closes `<st-dropdown>` when used in a shadow root [#223]
- Fixed a bug where scroll anchoring caused undesirable scrolling when `<st-details>` are grouped

Shoelace events were updated to use a lowercase, kebab-style naming convention. Instead of event names such as `slChange` and `slAfterShow`, you'll need to use `st-change` and `st-after-show` now.

This change was necessary to address a critical issue in frameworks that use DOM templates with declarative event bindings such as `<st-button @slChange="handler">`. Due to HTML's case-insensitivity, browsers translate attribute names to lowercase, turning `@slChange` into `@slchange`, making it impossible to listen to `slChange`.
...
- Added `input`, `label`, `prefix`, `clear-button`, `suffix`, `help-text` exported parts to `<st-select>` to make the input customizable
- Added toast notifications through the `toast()` method on `<st-alert>`
- Fixed a bug where mouse events would bubble up when `<st-button>` was disabled, causing tooltips to erroneously appear
- Fixed a bug where pressing space would open and immediately close `<st-dropdown>` panels in Firefox
- Fixed a bug where `<st-tooltip>` would throw an error on init
- Fixed a bug in custom keyframes animation example
- Refactored clear logic in `<st-input>`

## 2.0.0-beta.18

- Added `name` and `invalid` attribute to `<st-color-picker>`
- Added support for form submission and validation to `<st-color-picker>`
- Added touch support to demo resizers in the docs
- Added `<st-responsive-embed>` component
- Fixed a bug where swapping an animated element wouldn't restart the animation in `<st-animation>`
- Fixed a bug where the cursor was incorrect when `<st-select>` was disabled
- Fixed a bug where `slblur` and `slfocus` were emitted twice in `<st-select>`
- Fixed a bug where clicking on `<st-menu>` wouldn't focus it
- Fixed a bug in the popover utility where `onAfterShow` would fire too soon
- Fixed a bug where `bottom` and `right` placements didn't render properly in `<st-tab-group>`
- Improved keyboard logic in `<st-dropdown>`, `<st-menu>`, and `<st-select>`
- Updated `<st-animation>` to stable
- Updated to Stencil 2.0 (you may need to purge `node_modules` and run `npm install` after pulling)
- Updated entry points in `package.json` to reflect new filenames generated by Stencil 2

## 2.0.0-beta.17

- Added `minlength` and `spellcheck` attributes to `<st-textarea>`
- Fixed a bug where clicking a tag in `<st-select>` wouldn't toggle the menu
- Fixed a bug where options where `<st-select>` options weren't always visible or scrollable
- Fixed a bug where setting `null` on `<st-input>`, `<st-textarea>`, or `<st-select>` would throw an error
- Fixed a bug where `role` was on the wrong element and aria attribute weren't explicit in `<st-checkbox>`, `<st-switch>`, and `<st-radio>`
- Fixed a bug where dynamically adding/removing a slot wouldn't work as expected in `<st-card>`, `<st-dialog>`, and `<st-drawer>`
- Fixed a bug where the value wasn't updated and events weren't emitted when using `setRangeText` in `<st-input>` and `<st-textarea>`
...
- Added `required` to `<st-checkbox>`, `<st-select>`, and `<st-switch>`
- Added `reportValidity()` and `setCustomValidity()` methods to all form controls
- Added validation checking for custom and native form controls to `<st-form>`
- Added `novalidate` attribute to `<st-form>` to disable validation
...
- Added `hoist` attribute to `<st-color-picker>`, `<st-dropdown>`, and `<st-select>` to work around panel clipping
- Added `<st-format-bytes>` utility component
- Added `clearable` and `required` props to `<st-select>`
- Added `slclear` event to `<st-input>`
- Added keyboard support to the preview resizer in the docs
- Fixed a bug where the `aria-selected` state was incorrect in `<st-menu-item>`
- Fixed a bug where custom properties applied to `<st-tooltip>` didn't affect show/hide transitions
- Fixed a bug where `--st-input-color-*` custom properties had no effect on `<st-input>` and `<st-textarea>`
- Refactored `<st-dropdown>` and `<st-tooltip>` to use positioner elements so panels/tooltips can be customized easier

## 2.0.0-beta.15

- Added `image-comparer` component
- Added `--width`, `--height`, and `--thumb-size` custom props to `<st-switch>`
- Fixed an `aria-labelledby` attribute typo in a number of components
- Fixed a bug where the `change` event wasn't updating the value in `<st-input>`
- Fixed a bug where `<st-color-picker>` had the wrong border color in the dark theme
- Fixed a bug where `<st-menu-item>` had the wrong color in dark mode when disabled
- Fixed a bug where WebKit's autocomplete styles made inputs looks broken
- Fixed a bug where aria labels were wrong in `<st-select>`
- Fixed a bug where clicking the label wouldn't focus the control in `<st-select>`

## 2.0.0-beta.14

- Added dark theme
- Added `--st-panel-background-color` and `--st-panel-border-color` tokens
- Added `--tabs-border-color` custom property to `<st-tab-group>`
- Added `--track-color` custom property to `<st-range>`
- Added `tag` part to `<st-select>`
...
- Fixed a bug where the text color was being erroneously set in `<st-progress-ring>`
- Fixed a bug where `<st-progress-bar>` used the wrong part name internally for `indicator`
- Removed background color from `<st-menu>`
- Updated to Stencil 1.17.3

## 2.0.0-beta.13

- Added `slactivate` and `sldeactivate` events to `<st-menu-item>`
- Added experimental `<st-animation>` component
...
- Moved scrolling logic from `<st-menu>` to `<st-dropdown>`
...
- Fixed a bug where focus visible observer wasn't removed from `<st-details>`
...
- Replaced make-shift icon buttons with `<st-icon-button>` in alert, dialog, drawer, and tag
...
- Fixed a bug where nested form controls were ignored in `<st-form>`
