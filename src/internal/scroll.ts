...
export function lockBodyScrolling(lockingEl: HTMLElement) {
  ...
  if (!document.documentElement.classList.contains('st-scroll-lock')) {
    /** Scrollbar width + body padding calculation can go away once Safari has scrollbar-gutter support. */
    const scrollbarWidth = getScrollbarWidth() + getExistingBodyPadding(); // must be measured before the `st-scroll-lock` class is applied
    ...
    document.documentElement.style.setProperty('--st-scroll-lock-gutter', scrollbarGutterProperty);
    document.documentElement.classList.add('st-scroll-lock');
    document.documentElement.style.setProperty('--st-scroll-lock-size', `${scrollbarWidth}px`);
  }
}
...
export function unlockBodyScrolling(lockingEl: HTMLElement) {
  ...
  if (locks.size === 0) {
    ...
    document.documentElement.style.removeProperty('--st-scroll-lock-size');
  }
}
