...
import type StInput from './input.js';

describe('<st-input>', () => {
  it('should pass accessibility tests', async () => {
    const el = await fixture<StInput>(html` <st-input label="Name"></st-input> `);
  ...
  it('should have title if title attribute is set', async () => {
    const el = await fixture<StInput>(html` <st-input title="Test"></st-input> `);
  ...
  describe('value methods', () => {
    it('should set the value as a date when using valueAsDate', async () => {
      const el = document.createElement(`st-input`);
      el.type = 'date';
  ...
  describe('when using constraint validation', () => {
    ...
    it('should be invalid when required and disabled is removed', async () => {
      const el = await fixture<StInput>(html` <st-input disabled required></st-input> `);
      el.disabled = false;
    ...
    it('should receive the correct validation attributes ("states") when valid', async () => {
      const el = await fixture<StInput>(html` <st-input required value="a"></st-input> `);
    ...
    it('should receive validation attributes ("states") even when novalidate is used on the parent form', async () => {
      const el = await fixture<HTMLFormElement>(html` <form novalidate><st-input required></st-input></form> `);
      const input = el.querySelector<StInput>('st-input')!;
  ...
  describe('when submitting a form', () => {
    it('should serialize its name and value with FormData', async () => {
      const form = await fixture<HTMLFormElement>(html` <form><st-input name="a" value="1"></st-input></form> `);
    ...
    it('should submit the form when pressing enter in a form without a submit button', async () => {
      const form = await fixture<HTMLFormElement>(html` <form><st-input></st-input></form> `);
    ...
    it('should be invalid when setCustomValidity() is called with a non-empty value', async () => {
      const input = await fixture<HTMLFormElement>(html` <st-input></st-input> `);
    ...
    it('should be present in form data when using the form attribute and located outside of a <form>', async () => {
      const el = await fixture<HTMLFormElement>(html`
        <div>
          ...
          <st-input form="f" name="a" value="1"></st-input>
        </div>
      `);
  ...
  describe('when resetting a form', () => {
    it('should reset the element to its initial value', async () => {
      const form = await fixture<HTMLFormElement>(html`
        <form>
          <st-input name="a" value="test"></st-input>
          <st-button type="reset">Reset</st-button>
        </form>
      `);
  ...
  describe('when calling HTMLFormElement.reportValidity()', () => {
    it('should be invalid when the input is empty and form.reportValidity() is called', async () => {
      const form = await fixture<HTMLFormElement>(html`
        <form>
          <st-input required value=""></st-input>
          <st-button type="submit">Submit</st-button>
        </form>
      `);
  ...
  describe('when the value changes', () => {
    it('should emit st-change and st-input when the user types in the input', async () => {
      const el = await fixture<StInput>(html` <st-input></st-input> `);
      const inputHandler = sinon.spy();
      const changeHandler = sinon.spy();

      el.addEventListener('st-input', inputHandler);
      el.addEventListener('st-change', changeHandler);
      el.focus();
    ...
    it('should not emit st-change or st-input when the value is set programmatically', async () => {
      const el = await fixture<StInput>(html` <st-input></st-input> `);

      el.addEventListener('st-change', () => expect.fail('st-change should not be emitted'));
      el.addEventListener('st-input', () => expect.fail('st-input should not be emitted'));
      el.value = 'abc';

      await el.updateComplete;
    });

    it('should not emit st-change or st-input when calling setRangeText()', async () => {
      const el = await fixture<StInput>(html` <st-input value="hi there"></st-input> `);
  ...
  describe('when type="number"', () => {
    it('should be valid when the value is within the boundary of a step', async () => {
      const el = await fixture<StInput>(html` <st-input type="number" step=".5" value="1.5"></st-input> `);
      expect(el.checkValidity()).to.be.true;
    });

    it('should be invalid when the value is not within the boundary of a step', async () => {
      const el = await fixture<StInput>(html` <st-input type="number" step=".5" value="1.25"></st-input> `);
      expect(el.checkValidity()).to.be.false;
    });
    ...
    it('should increment by step when stepUp() is called', async () => {
      const el = await fixture<StInput>(html` <st-input type="number" step="2" value="2"></st-input> `);
    ...
    it('should not emit st-input or st-change when stepUp() is called programmatically', async () => {
      const el = await fixture<StInput>(html` <st-input type="number" step="2" value="2"></st-input> `);
    ...
    it('should not emit st-input and st-change when stepDown() is called programmatically', async () => {
      const el = await fixture<StInput>(html` <st-input type="number" step="2" value="2"></st-input> `);
  ...
  describe('when using spellcheck', () => {
    ...
    it('should enable spellcheck when set to "true"', async () => {
      const el = await fixture<StInput>(html` <st-input spellcheck="true"></st-input> `);
      const input = el.shadowRoot!.querySelector<HTMLInputElement>('input')!;
      expect(input.getAttribute('spellcheck')).to.equal('true');
      expect(input.spellcheck).to.be.true;
    });
    ...
    it('should disable spellcheck when set to "false"', async () => {
      const el = await fixture<StInput>(html` <st-input spellcheck="false"></st-input> `);
      const input = el.shadowRoot!.querySelector<HTMLInputElement>('input')!;
    ...
    it('should submit with the correct form when the form attribute changes', async () => {
      const el = await fixture<HTMLFormElement>(html`
        <div>
          ...
          <st-input form="f1" name="a" value="1"></st-input>
        </div>
      `);
      const form = el.querySelector<HTMLFormElement>('#f2')!;
      const input = document.querySelector('st-input')!;
    ...
    it('should return both native and Shoelace form controls in the correct DOM order', async () => {
      const el = await fixture<HTMLFormElement>(html`
        <div>
          ...
          <st-input type="text" name="b" value="2" form="f1"></st-input>
          <form id="f1">
            <input type="hidden" name="c" value="3" />
            <input type="text" name="d" value="4" />
            <st-input name="e" value="5"></st-input>
            <textarea name="f">6</textarea>
            <st-textarea name="g" value="7"></st-textarea>
            <st-checkbox name="h" value="8"></st-checkbox>
          </form>
          <input type="text" name="i" value="9" form="f1" />
          <st-input type="text" name="j" value="10" form="f1"></st-input>
        </div>
      `);
  ...
  describe('when using the setRangeText() function', () => {
    it('should set replacement text in the correct location', async () => {
      const el = await fixture<StInput>(html` <st-input value="test"></st-input> `);
  ...
  runFormControlBaseTests('st-input');
});
