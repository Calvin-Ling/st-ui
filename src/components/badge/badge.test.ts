...
import type StBadge from './badge.js';
...
describe('<st-badge>', () => {
  ...
  describe('when provided no parameters', () => {
    before(async () => {
      el = await fixture<StBadge>(html` <st-badge>Badge</st-badge> `);
    });
  ...
  describe('when provided a pill parameter', () => {
    before(async () => {
      el = await fixture<StBadge>(html` <st-badge pill>Badge</st-badge> `);
    });
  ...
  describe('when provided a pulse parameter', () => {
    before(async () => {
      el = await fixture<StBadge>(html` <st-badge pulse>Badge</st-badge> `);
    });
  ...
  ['primary', 'success', 'neutral', 'warning', 'danger'].forEach(variant => {
    describe(`when passed a variant attribute ${variant}`, () => {
      before(async () => {
        el = await fixture<StBadge>(html`<st-badge variant="${variant}">Badge</st-badge>`);
      });
