...
import type { SlErrorEvent } from '../../events/st-error.js';
import type { SlLoadEvent } from '../../events/st-load.js';
import type StIcon from './icon.js';
...
describe('<st-icon>', () => {
  ...
  describe('defaults ', () => {
    it('default properties', async () => {
      const el = await fixture<StIcon>(html` <st-icon></st-icon> `);
    ...
    it('renders pre-loaded system icons and emits st-load event', async () => {
      const el = await fixture<StIcon>(html` <st-icon library="system"></st-icon> `);
      const listener = oneEvent(el, 'st-load') as Promise<SlLoadEvent>;
    ...
    it('the icon is accessible', async () => {
      const el = await fixture<StIcon>(html` <st-icon library="system" name="check"></st-icon> `);
  ...
  describe('when a label is provided', () => {
    it('the icon has the correct default aria attributes', async () => {
      ...
      const el = await fixture<StIcon>(html` <st-icon label="${fakeLabel}" library="system" name="check"></st-icon> `);
  ...
  describe('when a valid src is provided', () => {
    it('the svg is rendered', async () => {
      ...
      const listener = oneEvent(el, 'st-load');
  ...
  describe('new library', () => {
    it('renders icons from the new library and emits st-load event', async () => {
      const el = await fixture<StIcon>(html` <st-icon library="test-library"></st-icon> `);
      const listener = oneEvent(el, 'st-load') as Promise<SlLoadEvent>;
    ...
    it('runs mutator from new library', async () => {
      const el = await fixture<StIcon>(html` <st-icon library="test-library" name="test-icon1"></st-icon> `);
      await elementUpdated(el);
  ...
  describe('negative cases', () => {
    ...
    it("svg not rendered with an icon that doesn't exist in the library", async () => {
      const el = await fixture<StIcon>(html` <st-icon library="test-library" name="does-not-exist"></st-icon> `);

      expect(el.shadowRoot?.querySelector('svg')).to.be.null;
    });

    it('emits st-error when the file cant be retrieved', async () => {
      const el = await fixture<StIcon>(html` <st-icon library="test-library"></st-icon> `);
      const listener = oneEvent(el, 'st-error') as Promise<SlErrorEvent>;
    ...
    it("emits st-error when there isn't an svg element in the registered icon", async () => {
      const el = await fixture<StIcon>(html` <st-icon library="test-library"></st-icon> `);
      const listener = oneEvent(el, 'st-error') as Promise<SlErrorEvent>;
  ...
  describe('svg sprite sheets', () => {
    ...
    it('Should properly grab an SVG and render it from bootstrap icons', async () => {
      ...
      const el = await fixture<StIcon>(html`<st-icon name="arrow-left" library="sprite"></st-icon>`);
    ...
    it('Should apply mutator to multiple identical spritesheet icons', async () => {
      ...
      const el = await fixture<HTMLDivElement>(html`
        <div>
          <st-icon name="arrow-left" library="sprite"></st-icon>
          <st-icon name="arrow-left" library="sprite"></st-icon>
        </div>
      `);

      const icons = [...el.querySelectorAll<StIcon>('st-icon')];
    ...
    it('Should render nothing if the sprite hash is wrong', async () => {
      ...
      const el = await fixture<StIcon>(html`<st-icon name="non-existent" library="sprite"></st-icon>`);
    ...
    it.skip("Should produce an error if the icon doesn't exist.", async () => {
      ...
      const el = await fixture<StIcon>(html`<st-icon name="bad-icon" library="sprite"></st-icon>`);
      const listener = oneEvent(el, 'st-error') as Promise<SlErrorEvent>;
