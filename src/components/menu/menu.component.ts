 ...
 *
 * @event {{ item: StMenuItem }} st-select - Emitted when a menu item is selected.
 */
export default class StMenu extends ShoelaceElement {
  ...
  private handleClick(event: MouseEvent) {
    ...
    const item = target as StMenuItem;

    if (item.type === 'checkbox') {
      item.checked = !item.checked;
    }

    this.emit('st-select', { detail: { item } });
  }
  ...
  private handleMouseDown(event: MouseEvent) {
    ...
    if (this.isMenuItem(target)) {
      this.setCurrentItem(target as StMenuItem);
    }
  }
  ...
  private isMenuItem(item: HTMLElement) {
    return (
      item.tagName.toLowerCase() === 'st-menu-item' ||
   ...
   */
  setCurrentItem(item: StMenuItem) {
    const items = this.getAllItems();
