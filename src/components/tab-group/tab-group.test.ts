...
import type { SlTabShowEvent } from '../../events/st-tab-show.js';
...
import type StTabPanel from '../tab-panel/tab-panel.js';
import type StTab from '../tab/tab.js';
import type StTabGroup from './tab-group.js';
...
const waitForScrollButtonsToBeRendered = async (tabGroup: StTabGroup): Promise<void> => {
  await waitUntil(() => {
    const scrollButtons = tabGroup.shadowRoot?.querySelectorAll('st-icon-button');
    return scrollButtons?.length === 2;
  });
};

const getClientRectangles = (tabGroup: StTabGroup): ClientRectangles => {
...
const expectHeaderToBeVisible = (container: HTMLElement, dataTestId: string): void => {
  const generalHeader = queryByTestId<StTab>(container, dataTestId);
...
const expectOnlyOneTabPanelToBeActive = async (container: HTMLElement, dataTestIdOfActiveTab: string) => {
  await waitUntil(() => {
    const tabPanels = Array.from(container.getElementsByTagName('st-tab-panel'));
    const activeTabPanels = tabPanels.filter((element: StTabPanel) => element.hasAttribute('active'));
...
const waitForHeaderToBeActive = async (container: HTMLElement, headerTestId: string): Promise<StTab> => {
  const generalHeader = queryByTestId<StTab>(container, headerTestId);
  await waitUntil(() => {
...
describe('<st-tab-group>', () => {
  ...
  it('should not throw error when unmounted too fast', async () => {
    ...
    el.innerHTML = '<st-tab-group></st-tab-group>';
    el.innerHTML = '';
  });
  ...
  it('displays all tabs', async () => {
    const tabGroup = await fixture<StTabGroup>(html`
      <st-tab-group>
        <st-tab slot="nav" panel="general" data-testid="general-tab-header">General</st-tab>
        <st-tab slot="nav" panel="disabled" disabled data-testid="disabled-tab-header">Disabled</st-tab>
        <st-tab-panel name="general">This is the general tab panel.</st-tab-panel>
  ...
  it('shows the first tab to be active by default', async () => {
    const tabGroup = await fixture<StTabGroup>(html`
      <st-tab-group>
        ...
        <st-tab-panel name="general" data-testid="general-tab-content">This is the general tab panel.</st-tab-panel>
  ...
  describe('scrolling behavior', () => {
    const generateTabs = (n: number): HTMLTemplateResult[] => {
      ...
      for (let i = 0; i < n; i++) {
        result.push(
          html`<st-tab slot="nav" panel="tab-${i}">Tab ${i}</st-tab>
            <st-tab-panel name="tab-${i}">Content of tab ${i}0</st-tab-panel> `
        );
      }
      return result;
    };
    ...
    it('shows scroll buttons on too many tabs', async () => {
      const tabGroup = await fixture<StTabGroup>(html`<st-tab-group> ${generateTabs(30)} </st-tab-group>`);
    ...
    it('does not show scroll buttons if all tabs fit on the screen', async () => {
      const tabGroup = await fixture<StTabGroup>(html`<st-tab-group> ${generateTabs(2)} </st-tab-group>`);
    ...
    it.skip('does not show scroll buttons if placement is start', async () => {
      const tabGroup = await fixture<StTabGroup>(html`<st-tab-group> ${generateTabs(50)} </st-tab-group>`);
      tabGroup.placement = 'start';
    ...
    it.skip('does scroll on scroll button click', async () => {
      ...
      const tabGroup = await fixture<StTabGroup>(
        html`<st-tab-group> ${generateTabs(numberOfElements)} </st-tab-group>`
      );
      ...
      const scrollToRightButton = tabGroup.shadowRoot?.querySelector('st-icon-button[part*="scroll-button--end"]');
  ...
  describe('tab selection', () => {
    const expectCustomTabToBeActiveAfter = async (tabGroup: StTabGroup, action: () => Promise<void>): Promise<void> => {
      const generalHeader = await waitForHeaderToBeActive(tabGroup, 'general-header');
      generalHeader.focus();

      const customHeader = queryByTestId<StTab>(tabGroup, 'custom-header');
      expect(customHeader).not.to.have.attribute('active');

      const showEventPromise = oneEvent(tabGroup, 'st-tab-show') as Promise<SlTabShowEvent>;
      await action();
    ...
    const expectGeneralTabToBeStillActiveAfter = async (
      ...
      oneEvent(tabGroup, 'st-tab-show').then(() => (showEventFired = true));
      oneEvent(tabGroup, 'st-tab-hide').then(() => (hideEventFired = true));
      await action();
    ...
    it('selects a tab by clicking on it', async () => {
      const tabGroup = await fixture<StTabGroup>(html`
        <st-tab-group>
          <st-tab slot="nav" panel="general" data-testid="general-header">General</st-tab>
          <st-tab slot="nav" panel="custom" data-testid="custom-header">Custom</st-tab>
          <st-tab-panel name="general">This is the general tab panel.</st-tab-panel>
          <st-tab-panel name="custom" data-testid="custom-tab-content">This is the custom tab panel.</st-tab-panel>
        </st-tab-group>
      `);
    ...
    it('does not change if a disabled tab is clicked', async () => {
      const tabGroup = await fixture<StTabGroup>(html`
        <st-tab-group>
          <st-tab slot="nav" panel="general" data-testid="general-header">General</st-tab>
          <st-tab slot="nav" panel="disabled" data-testid="disabled-header" disabled>disabled</st-tab>
          <st-tab-panel name="general" data-testid="general-tab-content">This is the general tab panel.</st-tab-panel>
          <st-tab-panel name="disabled">This is the disabled tab panel.</st-tab-panel>
        </st-tab-group>
      `);
    ...
    it('does not allow selection of disabled tabs with arrow keys', async () => {
      const tabGroup = await fixture<StTabGroup>(html`
        <st-tab-group>
          ...
          <st-tab-panel name="general" data-testid="general-tab-content">This is the general tab panel.</st-tab-panel>
          <st-tab-panel name="disabled">This is the custom tab panel.</st-tab-panel>
        </st-tab-group>
      `);
