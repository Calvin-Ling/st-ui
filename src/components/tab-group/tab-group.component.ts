...
import StResizeObserver from '../resize-observer/resize-observer.component.js';
 ...
 * @since 2.0
 *
 * @dependency st-icon-button
 *
 * @slot - Used for grouping tab panels in the tab group. Must be `<st-tab-panel>` elements.
 * @slot nav - Used for grouping tabs in the tab group. Must be `<st-tab>` elements.
 *
 * @event {{ name: String }} st-tab-show - Emitted when a tab is shown.
 * @event {{ name: String }} st-tab-hide - Emitted when a tab is hidden.
 *
 ...
 * @csspart scroll-button - The previous/next scroll buttons that show when tabs are scrollable, an `<st-icon-button>`.
 ...
 */
export default class StTabGroup extends ShoelaceElement {
  static styles: CSSResultGroup = [componentStyles, styles];
  static dependencies = { 'st-icon-button': SlIconButton, 'st-resize-observer': StResizeObserver };

  private activeTab?: StTab;
  private mutationObserver: MutationObserver;
  private resizeObserver: ResizeObserver;
  private tabs: StTab[] = [];
  private focusableTabs: StTab[] = [];
  private panels: StTabPanel[] = [];
  ...
  connectedCallback() {
    const whenAllDefined = Promise.all([
      customElements.whenDefined('st-tab'),
      customElements.whenDefined('st-tab-panel')
    ]);
    ...
    this.mutationObserver = new MutationObserver(mutations => {
      ...
      const instanceMutations = mutations.filter(({ target }) => {
        ...
        if ((target as HTMLElement).closest('st-tab-group') !== this) return false; // We are not direct children

        // We should only care about changes to the tab or tab panel
        const tagName = (target as HTMLElement).tagName.toLowerCase();
        return tagName === 'st-tab' || tagName === 'st-tab-panel';
      });
      ...
      if (instanceMutations.some(m => m.attributeName === 'disabled')) {
        ...
        const tabs = instanceMutations
          .filter(m => m.attributeName === 'active' && (m.target as HTMLElement).tagName.toLowerCase() === 'st-tab')
          .map(m => m.target as StTab);
  ...
  private getAllTabs() {
    ...
    return slot.assignedElements() as StTab[];
  }

  private getAllPanels() {
    return [...this.body.assignedElements()].filter(el => el.tagName.toLowerCase() === 'st-tab-panel') as [StTabPanel];
  }
  ...
  private handleClick(event: MouseEvent) {
    ...
    const tab = target.closest('st-tab');
  ...
  private handleKeyDown(event: KeyboardEvent) {
    ...
    if (['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown', 'Home', 'End'].includes(event.key)) {
      ...
      let nextTab: null | StTab = null;

      if (activeEl?.tagName.toLowerCase() === 'st-tab') {
        if (event.key === 'Home') {
  ...
  private setActiveTab(tab: StTab, options?: { emitEvents?: boolean; scrollBehavior?: 'auto' | 'smooth' }) {
    options = {
      emitEvents: true,
      scrollBehavior: 'auto',
      ...options
    ...
    if (tab !== this.activeTab && !tab.disabled) {
      ...
      if (options.emitEvents) {
        if (previousTab) {
          this.emit('st-tab-hide', { detail: { name: previousTab.panel } });
        }

        this.emit('st-tab-show', { detail: { name: this.activeTab.panel } });
      }
    }
  }
        ...
        : ''}
          ...
          <div class="tab-group__nav" @scrollend=${this.updateScrollButtons}>
            <div part="tabs" class="tab-group__tabs" role="tablist">
              ...
              <st-resize-observer @st-resize=${this.syncIndicator}>
