...
import StCheckbox from '../checkbox/checkbox.component.js';
 ...
 * @since 2.0
 *
 * @dependency st-checkbox
 * @dependency st-icon
 * @dependency st-spinner
 *
 * @event st-expand - Emitted when the tree item expands.
 * @event st-after-expand - Emitted after the tree item expands and all animations are complete.
 * @event st-collapse - Emitted when the tree item collapses.
 * @event st-after-collapse - Emitted after the tree item collapses and all animations are complete.
 * @event st-lazy-change - Emitted when the tree item's lazy state changes.
 * @event st-lazy-load - Emitted when a lazy item is selected. Use this event to asynchronously load data and append
 ...
 */
export default class StTreeItem extends ShoelaceElement {
  static styles: CSSResultGroup = [componentStyles, styles];
  static dependencies = {
    'st-checkbox': StCheckbox,
    'st-icon': StIcon,
    'st-spinner': SlSpinner
  };
  ...
  private async animateCollapse() {
    ...
    this.emit('st-after-collapse');
  }

  // Checks whether the item is nested into an item
  private isNestedItem(): boolean {
    const parent = this.parentElement;
    return !!parent && StTreeItem.isTreeItem(parent);
  }
  ...
  protected willUpdate(changedProperties: PropertyValueMap<StTreeItem> | Map<PropertyKey, unknown>) {
  ...
  handleLazyChange() {
    this.emit('st-lazy-change');
  }

  /** Gets all the nested tree items in this node. */
  getChildrenItems({ includeDisabled = true }: { includeDisabled?: boolean } = {}): StTreeItem[] {
    return this.childrenSlot
      ? ([...this.childrenSlot.assignedElements({ flatten: true })].filter(
        (item: StTreeItem) => StTreeItem.isTreeItem(item) && (includeDisabled || !item.disabled)
      ) as StTreeItem[])
      : [];
  }
    ...
    return html`
      ...
      'tree-item__expand-button--visible': showExpandButton
          ...
          >
            ${when(
      this.loading,
      () => html` <st-spinner part="spinner" exportparts="base:spinner__base"></st-spinner> `
    )}
            <slot class="tree-item__expand-icon-slot" name="expand-icon">
              <st-icon library="system" name=${isRtl ? 'chevron-left' : 'chevron-right'}></st-icon>
            </slot>
