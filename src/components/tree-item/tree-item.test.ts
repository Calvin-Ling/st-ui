...
import type StTreeItem from './tree-item.js';

describe('<st-tree-item>', () => {
  let leafItem: StTreeItem;
  let parentItem: StTreeItem;

  beforeEach(async () => {
    leafItem = await fixture(html` <st-tree-item>Node 1</st-tree-item> `);
  ...
  describe('when the user clicks the expand button', () => {
    describe('and the item is collapsed', () => {
      it('should emit st-expand and st-after-expand events', async () => {
        // Arrange
        const expandSpy = sinon.spy();
        const afterExpandSpy = sinon.spy();

        parentItem.addEventListener('st-expand', expandSpy);
        parentItem.addEventListener('st-after-expand', afterExpandSpy);
    ...
    describe('and the item is expanded', () => {
      it('should emit st-collapse and st-after-collapse events', async () => {
        // Arrange
        const collapseSpy = sinon.spy();
        const afterCollapseSpy = sinon.spy();

        parentItem.addEventListener('st-collapse', collapseSpy);
        parentItem.addEventListener('st-after-collapse', afterCollapseSpy);

        parentItem.expanded = true;
        await oneEvent(parentItem, 'st-after-expand');
  ...
  describe('when the item is lazy', () => {
    it('should emit st-lazy-change when the lazy attribute is added and removed', async () => {
      // Arrange
      const lazyChangeSpy = sinon.spy();

      parentItem.addEventListener('st-lazy-change', lazyChangeSpy);
