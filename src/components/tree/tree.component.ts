...
import StTreeItem from '../tree-item/tree-item.component.js';
import styles from './tree.styles.js';

function syncCheckboxes(changedTreeItem: StTreeItem, initialSync = false) {
  function syncParentItem(treeItem: StTreeItem) {
  ...
  function syncAncestors(treeItem: StTreeItem) {
    const parentItem: StTreeItem | null = treeItem.parentElement as StTreeItem;

    if (StTreeItem.isTreeItem(parentItem)) {
      syncParentItem(parentItem);
      syncAncestors(parentItem);
    }
  }

  function syncDescendants(treeItem: StTreeItem) {
 ...
 * @since 2.0
 *
 * @event {{ selection: StTreeItem[] }} st-selection-change - Emitted when a tree item is selected or deselected.
 *
 * @slot - The default slot.
 * @slot expand-icon - The icon to show when the tree item is expanded. Works best with `<st-icon>`.
 * @slot collapse-icon - The icon to show when the tree item is collapsed. Works best with `<st-icon>`.
 *
 * @csspart base - The component's base wrapper.
 *
 * @cssproperty [--indent-size=var(--st-spacing-medium)] - The size of the indentation for nested items.
 * @cssproperty [--indent-guide-color=var(--st-color-neutral-200)] - The color of the indentation line.
 * @cssproperty [--indent-guide-offset=0] - The amount of vertical spacing to leave between the top and bottom of the
 *  indentation line's starting position.
 * @cssproperty [--indent-guide-style=solid] - The style of the indentation line, e.g. solid, dotted, dashed.
 * @cssproperty [--indent-guide-width=0] - The width of the indentation line.
 */
export default class StTree extends ShoelaceElement {
  ...
  //
  private lastFocusedItem: StTreeItem | null;
  private mutationObserver: MutationObserver;
  private clickTarget: StTreeItem | null = null;
  ...
  constructor() {
    ...
    this.addEventListener('st-lazy-change', this.handleSlotChange);
  }
  ...
  private initTreeItem = (item: StTreeItem) => {
  ...
  private handleTreeChanged = (mutations: MutationRecord[]) => {
    for (const mutation of mutations) {
      const addedNodes: StTreeItem[] = [...mutation.addedNodes].filter(StTreeItem.isTreeItem) as StTreeItem[];
      const removedNodes = [...mutation.removedNodes].filter(StTreeItem.isTreeItem) as StTreeItem[];
  ...
  private selectItem(selectedItem: StTreeItem) {
    ...
    if (
      ...
      Promise.all(nextSelection.map(el => el.updateComplete)).then(() => {
        this.emit('st-selection-change', { detail: { selection: nextSelection } });
      });
    }
  }

  private getAllTreeItems() {
    return [...this.querySelectorAll<StTreeItem>('st-tree-item')];
  }

  private focusItem(item?: StTreeItem | null) {
    item?.focus();
  }
  ...
  private handleKeyDown(event: KeyboardEvent) {
    ...
    if (items.length > 0) {
      ...
      const activeItem: StTreeItem | undefined = items[activeItemIndex];
  ...
  private handleClick(event: Event) {
    const target = event.target as StTreeItem;
    const treeItem = target.closest('st-tree-item')!;
  ...
  handleMouseDown(event: MouseEvent) {
    ...
    this.clickTarget = event.target as StTreeItem;
  }
  ...
  private handleFocusIn = (event: FocusEvent) => {
    ...
    if (StTreeItem.isTreeItem(target) && !target.disabled) {
  ...
  async handleSelectionChange() {
    ...
    if (isSelectionMultiple) {
      ...
      [...this.querySelectorAll(':scope > st-tree-item')].forEach((treeItem: StTreeItem) =>
  ...
  get selectedItems(): StTreeItem[] {
    const items = this.getAllTreeItems();
    const isSelected = (item: StTreeItem) => item.selected;
  ...
  getFocusableItems() {
    ...
    return items.filter(item => {
      ...
      const parent: StTreeItem | null | undefined = item.parentElement?.closest('[role=treeitem]');
