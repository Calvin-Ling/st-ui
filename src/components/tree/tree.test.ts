...
import type StTreeItem from '../tree-item/tree-item.js';
import type StTree from './tree.component.js';
...
describe('<st-tree>', () => {
  ...
  beforeEach(async () => {
    el = await fixture(html`
      <st-tree>
        <st-tree-item>Node 1</st-tree-item>
        <st-tree-item>Node 2</st-tree-item>
        <st-tree-item id="expandable">
          Parent Node
          <st-tree-item>Child Node 1</st-tree-item>
          <st-tree-item>
            Child Node 2
            <st-tree-item>Child Node 2 - 1</st-tree-item>
            <st-tree-item>Child Node 2 - 2</st-tree-item>
          </st-tree-item>
        </st-tree-item>
        <st-tree-item>Node 3</st-tree-item>
      </st-tree>
    `);
  });
  ...
  it('should not focus collapsed nodes', async () => {
    // Arrange
    const parentNode = el.children[2] as StTreeItem;
    const childNode = parentNode.children[1] as StTreeItem;
  ...
  describe('when a custom expanded/collapsed icon is provided', () => {
    ...
    it('should append a clone of the icon in the proper slot of the tree item', async () => {
      ...
      // Act
      const treeItems = [...el.querySelectorAll('st-tree-item')];
  ...
  describe('Keyboard navigation', () => {
    ...
    describe('when Home is pressed', () => {
      it('should move the focus to the first tree item in the tree', async () => {
        // Arrange
        const parentNode = el.children[3] as StTreeItem;
        parentNode.focus();
        await el.updateComplete;
    ...
    describe('when End is pressed', () => {
      it('should move the focus to the last tree item in the tree', async () => {
        // Arrange
        const parentNode = el.children[0] as StTreeItem;
        parentNode.focus();
        await el.updateComplete;
    ...
    describe('when Enter is pressed', () => {
      describe('and selection is "single"', () => {
        it('should select only one tree item', async () => {
          // Arrange
          el.selection = 'single';
          const node = el.children[1] as StTreeItem;
          node.focus();
          await el.updateComplete;
      ...
      describe('and selection is "leaf"', () => {
        it('should select only one tree item', async () => {
          // Arrange
          el.selection = 'leaf';
          const node = el.children[0] as StTreeItem;
          node.focus();
          await el.updateComplete;
  ...
  describe('Interactions', () => {
    ...
    describe('when the user clicks the expand button', () => {
      it('should expand the tree item', async () => {
        ...
        const node = el.children[2] as StTreeItem;
    ...
    describe('when the user clicks on a tree item', () => {
      describe('and selection is "single"', () => {
        it('should select only one tree item', async () => {
          // Arrange
          el.selection = 'single';
          const node0 = el.children[0] as StTreeItem;
          const node1 = el.children[1] as StTreeItem;
      ...
      describe('and selection is "multiple"', () => {
        ...
        it('should set the indeterminate state to tree items if a child is selected', async () => {
          ...
          const parentNode = el.children[2] as StTreeItem;
          const childNode = parentNode.children[0] as StTreeItem;
    ...
    describe('when selection is "single"', () => {
      describe('and user clicks on same item twice', () => {
        it('should emit `st-selection-change` event once', async () => {
          // Arrange
          el.selection = 'single';
          await el.updateComplete;

          const selectedChangeSpy = sinon.spy();
          el.addEventListener('st-selection-change', selectedChangeSpy);
  ...
  describe('when selection is "leaf"', () => {
    ...
    describe('and user clicks on expandable item', () => {
      it('should not emit `st-selection-change` event', async () => {
        // Arrange
        el.selection = 'leaf';
        await el.updateComplete;
        ...
        const node = el.querySelector<StTreeItem>('#expandable')!;
  ...
  describe('when selection is "multiple"', () => {
    describe('and user clicks on same item twice', () => {
      it('should emit `st-selection-change` event twice', async () => {
        // Arrange
        el.selection = 'multiple';
        await el.updateComplete;
    ...
    describe('when the tree gets initialized', () => {
      describe('and a parent node is selected', () => {
        it('should select all the nested children', async () => {
          // Arrange
          const tree = await fixture<StTree>(html`
            <st-tree selection="multiple">
              <st-tree-item selected>
                Parent Node
                <st-tree-item selected>Child Node 1</st-tree-item>
                <st-tree-item>
                  Child Node 2
                  <st-tree-item>Child Node 2 - 1</st-tree-item>
                  <st-tree-item>Child Node 2 - 2</st-tree-item>
          ...
          `);
          const treeItems = Array.from<StTreeItem>(tree.querySelectorAll('st-tree-item'));
  ...
  it("Should not render 'null' if it can't find a custom icon", async () => {
    const tree = await fixture<StTree>(html`
      <st-tree>
        <st-tree-item>
          Item 1
          <st-icon name="1-circle" slot="expand-icon"></st-icon>
