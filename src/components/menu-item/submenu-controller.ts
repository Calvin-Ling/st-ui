...
import type StPopup from '../popup/popup.js';
...
export class SubmenuController implements ReactiveController {
  private host: ReactiveControllerHost & StMenuItem;
  private popupRef: Ref<StPopup> = createRef();
  ...
  constructor(host: ReactiveControllerHost & StMenuItem, hasSlotController: HasSlotController) {
  ...
  private addListeners() {
    ...
    if (!this.isPopupConnected) {
      if (this.popupRef.value) {
        ...
        this.popupRef.value.addEventListener('st-reposition', this.handlePopupReposition);
  ...
  private removeListeners() {
    ...
    if (this.isPopupConnected) {
      if (this.popupRef.value) {
        ...
        this.popupRef.value.removeEventListener('st-reposition', this.handlePopupReposition);
  ...
  private handleSubmenuEntry(event: KeyboardEvent) {
    ...
    for (const elt of submenuSlot.assignedElements()) {
      menuItems = elt.querySelectorAll("st-menu-item, [role^='menuitem']");
  ...
  private handleClick = (event: MouseEvent) => {
    ...
    if (event.target === this.host) {
      ...
      (event.target.tagName === 'st-menu-item' || event.target.role?.startsWith('menuitem'))
    ) {
      this.disableSubmenu();
    }
  };
  ...
  private handlePopupReposition = () => {
    const submenuSlot: HTMLSlotElement | null = this.host.renderRoot.querySelector("slot[name='submenu']");
    const menu = submenuSlot?.assignedElements({ flatten: true }).filter(el => el.localName === 'st-menu')[0];
  ...
  renderSubmenu() {
    ...
    // Always render the slot, but conditionally render the outer <st-popup>
    if (!this.isConnected) {
